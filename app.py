...
def add():
    ...
    if emp_id==''or emp_name==''or mob_no==''or dept==''or emp_salary=='':
        ...
        conn=pymysql.connect(user='root',host='localhost',password='Aniket@2000',database='emp_project')
...
def update():
    ...
    if emp_id==''or emp_name==''or mob_no==''or dept==''or emp_salary=='':
        ...
        conn=pymysql.connect(user='root',host='localhost',password='Aniket@2000',database='emp_project')
        qur=f'''UPDATE emp SET emp_id="{emp_id}",emp_name="{emp_name}",mob_no="{mob_no}",
        dept="{dept}",emp_salary="{emp_salary}" 
        WHERE emp_id="{emp_id}"
        '''
        mycur=conn.cursor()
        mycur.execute(qur)
